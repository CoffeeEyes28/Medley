...
const reactId = reacted.map((id) => id._id)
    ...
    return (
     ...
     ?   <IconButton disabled={allowDelete} onClick={() =>  remove(reactId[0], userData._id) } ><FavoriteIcon sx={{color: 'red'}}/></IconButton> : !allowDelete && <IconButton onClick={() => reaction(userData._id, selfData.username) }><FavoriteBorderIcon /></IconButton>}
